  ...
  const updateService = useCallback(
    async (serviceId: number, data: ServiceUpdateData) => {
      try {
        ...
        const hasImageFile = data.service_image instanceof File;
        const shouldDeleteImage = data.service_image === null;

        if (hasImageFile) {
          const formData = new FormData();

          Object.entries(data).forEach(([key, value]) => {
            if (key === 'service_image' && value instanceof File) {
              formData.append(key, value);
            } else if (key === 'meeting_locations' && Array.isArray(value)) {
          ...
          await apiClient.services.deleteImage(serviceId);

          const { service_image, ...updateData } = data;
          if (Object.keys(updateData).length > 0) {
            const response = await apiClient.services.update(
              serviceId,
              updateData
            );
            return response;
